...
function paginate_next($label = '', $attr = array(), $max_page = 0)
{
	...
	if ( ! is_single() and ($nextpage <= $max_page))
	{
		...
		return html_tag('li', $attr, html_tag('a', array(), $label));
	}
}
...
function paginate($args = array())
{
	...
	if ( ! empty($nums))
	{
		...
		{
			if ($k === 'current') {
				$list .= html_tag('li', array('class' => 'active'), $r);
			}
			elseif (isset($r['dot'])) {
		...
		$html = html_tag('ul', array('class' => 'nav-pager'), $list);

		$output = html_tag('nav', array('class' => 'archive', 'id' => 'wrap-pager'), $html);
